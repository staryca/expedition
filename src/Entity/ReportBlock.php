...
class ReportBlock
{
    ...
    {
        $searchIndex = $this->userNotes ? $this->userNotes . '. ' : '';
        ...
        if ($this->organization) {
            $org = $this->organization->getName() . ', ' . $this->organization->getAddress() . ', '
                . $this->organization->getDescription() . '. ' . $this->organization->getNotes() . '. ';
            if ($this->organization->getOrganizationInformants()->count() > 0) {
                $text = 'Удзельнікі: ';
                foreach ($this->organization->getOrganizationInformants() as $organizationInformant) {
        ...
        if ($this->getDescription() !== null) {
            $searchIndex .= trim($this->getDescription(), " .,;\t\n\r\0\x0B") . '. ';
        }

        if ($this->getContentFile() !== null) {
            $searchIndex .= 'Змест: ' . $this->getContentFile()->getSearchIndex() . '. ';
        }

        if ($this->getRealFiles()->count() > 0) {
            $text = 'Файлы: ';
            foreach ($this->getRealFiles() as $file) {
                $text .= $file->getSearchIndex() . '. ';
            }
            $text = trim($text, " ,;");
            $searchIndex .= $text . '. ';
        }

        if (!empty($this->getPhotoNotes())) {
            $searchIndex .= 'Заўвагі к фота: ' . trim($this->getPhotoNotes(), " .,;\t\n\r\0\x0B") . '. ';
        }

        if (!empty($this->getVideoNotes())) {
            $searchIndex .= 'Заўвагі к відэа: ' . trim($this->getVideoNotes(), " .,;\t\n\r\0\x0B") . '. ';
        }

        $fileMarkers = $this->getFileMarkerGroups();
        if (!empty($fileMarkers)) {
            $text = 'Файлы: ';
            $files = $this->getFilesOfMarkers();
            foreach ($files as $file) {
                $text .= $file->getSearchIndex(true, $fileMarkers[$file->getId()]) . '. ';
            }
            $searchIndex .= $text . '. ';
        }
